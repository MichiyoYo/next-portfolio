...
export function MagicButton({ title, icon, onClick }: MagicButtonProps) {
  return (
    ...
    >
      ...
      <span className='gap-1 inline-flex h-full w-full cursor-pointer items-center justify-center rounded-lg bg-slate-950 px-7 py-1 text-sm font-medium text-white backdrop-blur-3xl'>
        {title} {icon}
      </span>
    </button>
  );
